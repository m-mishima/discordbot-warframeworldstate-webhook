<?php

include __DIR__ . '/readconfig.php';

$missiontypelist = null;        $missiontypelist_filedate = null;
$regionlist = null;             $regionlist_filedate = null;
$solnodelist = null;            $solnodelist_filedate = null;
$sortiebosslist = null;         $sortiebosslist_filedate = null;
$sortiemodifierlist = null;     $sortiemodifierlist_filedate = null;
$fissurefilterlist = null;      $fissurefilterlist_filedate = null;
$voidrelictierlist = null;      $voidrelictierlist_filedate = null;
$timezone = null;               $timezone_filedate = null;
$worldstateurl = null;          $worldstateurl_filedate = null;
$webhookurl_sortie = null;      $webhookurl_sortie_filedate = null;
$webhookurl_fissure = null;     $webhookurl_fissure_filedate = null;
$webhookurl_nicefissure = null; $webhookurl_nicefissure_filedate = null;
$webhookurl_devel = null;       $webhookurl_devel_filedate = null;
$itemtranslatelist = null;      $itemtranslatelist_filedate = null;

readconfig();

//var_dump( $missiontypelist );
//var_dump( $missiontypelist_filedate );
//var_dump( $regionlist );
//var_dump( $regionlist_filedate );
//var_dump( $solnodelist );
//var_dump( $solnodelist_filedate );
//var_dump( $sortiebosslist );
//var_dump( $sortiebosslist_filedate );
//var_dump( $sortiemodifierlist );
//var_dump( $sortiemodifierlist_filedate );
//var_dump( $fissurefilterlist );
//var_dump( $fissurefilterlist_filedate );
//var_dump( $voidrelictierlist );
//var_dump( $voidrelictierlist_filedate );
//var_dump( $timezone );
//var_dump( $timezone_filedate );
//var_dump( $worldstateurl );
//var_dump( $worldstateurl_filedate );
//var_dump( $webhookurl_sortie );
//var_dump( $webhookurl_sortie_filedate );
//var_dump( $webhookurl_fissure );
//var_dump( $webhookurl_fissure_filedate );
//var_dump( $webhookurl_nicefissure );
//var_dump( $webhookurl_nicefissure_filedate );
//var_dump( $webhookurl_devel );
//var_dump( $webhookurl_devel_filedate );
//var_dump( $itemtranslatelist );
//var_dump( $itemtranslatelist_filedate );

?>
